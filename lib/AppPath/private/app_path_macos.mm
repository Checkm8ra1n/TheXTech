...
// iOS: get the Documents directory path
#if TARGET_OS_IPHONE || TARGET_OS_SIMULATOR
#include <Foundation/Foundation.h>
static std::string s_iosDocumentsPath;
static std::string getIOSDocumentsPath()
{
    if(!s_iosDocumentsPath.empty())
        return s_iosDocumentsPath;
    @autoreleasepool {
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        if([paths count] > 0) {
            s_iosDocumentsPath = std::string([[paths objectAtIndex:0] UTF8String]) + "/";
        }
    }
    return s_iosDocumentsPath;
}
...
#if TARGET_OS_IPHONE || TARGET_OS_SIMULATOR
    // On iOS, use Documents directory for both assets and user data
    // The user must copy game assets into the app's Documents folder
    {
        std::string docsPath = getIOSDocumentsPath();
        if(!docsPath.empty())
        {
            // User data goes to Documents/TheXTech/
            s_userDirectory = docsPath;
            // Assets are looked for in Documents/assets/ if present
            std::string assetsCandidate = docsPath + "assets/";
            if(DirMan::exists(assetsCandidate))
                s_assetsRoot = assetsCandidate;
            else
                s_assetsRoot = docsPath; // fallback: game files directly in Documents
        }
    }
#endif
